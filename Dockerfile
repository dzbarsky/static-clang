FROM ubuntu
RUN apt-get update && apt-get install -y ca-certificates curl git python3 binutils
...
RUN echo "bazel build -c opt --config remote @llvm-project//clang" > ~/.bash_history
